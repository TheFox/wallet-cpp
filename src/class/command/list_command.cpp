...
namespace Wallet
{
  int ListCommand::execute()
  {
    ...
    listEntries(wallet.getEntries(date, this->options.category, this->options.epicHandle));
  ...
  void listEntries(const Container::EntryContainer& container)
  {
    ...
    std::cout
      ...
      << " Date          Revenue    Expense    Balance    Category    Epic   Title"
      << std::endl;
    ...
    for (const auto& yearPair : container.years) {
      ...
      for (const auto& entry : dayPair.second.entries) {
        ...
        std::cout
          ...
          << ' ' << std::setw(7) << std::right << entry.epicHandle
          << "   " << entry.title
          << std::endl;
      }
    }
